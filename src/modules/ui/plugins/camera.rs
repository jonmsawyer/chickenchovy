...
fn setup_camera(mut commands: Commands) {
    commands.spawn(Camera2dBundle::default()).insert(MainCamera::default());
}
...
fn pan_camera(
    ...
    let (_projection, _camera_transform) = query.single();
    ...
    let current_pos = match windows.single().cursor_position() {
    ...
    for (_projection2, mut transform) in query.iter_mut() {
...
impl Plugin for CameraControllerPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<CameraPanned>()
            ...
            // .add_system(OnUpdate(GameState::Next, pan_camera));
            // .add_systems(OnEnter(AppState::Setup), load_textures)
            .add_system(pan_camera);
            // .add_systems(OnEnter(GameState::Next), pan_camera);
